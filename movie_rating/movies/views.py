    ...
    if request.method == 'POST':
        ...
        for movie in user_rated_movies:
            if movie.name_eg in request.POST:
                User_Rating.objects.get(
                    user=current_user, movie=movie).delete()
        ...
        for data in request.POST:
            ...
            rating = User_Rating(user=current_user, movie=movie,
                                 rating=float(request.POST.get(data)))
            rating.save()
        ...
        l_m_english = list(Movie.objects.filter(
            is_english=True).order_by('-date')[:100])
        p_m_english = list(Movie.objects.filter(
            is_english=True).order_by('-final_rating')[:100])
        l_m_arabic = list(Movie.objects.filter(
            is_english=False).order_by('-date')[:100])
        p_m_arabic = list(Movie.objects.filter(
            is_english=False).order_by('-final_rating')[:100])
        ...
        for movie in p_m_english:
            if (movie not in user_rated_movies) and (movie not in latest_movies_english):
                popular_movies_english.insert(
                    len(popular_movies_english), movie)
        ...
        num_samples = 20
        popular_movies_english = random.sample(
            popular_movies_english, num_samples)
        popular_movies_arabic = random.sample(
            popular_movies_arabic, num_samples)
...
class SearchResultsView(LoginRequiredMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        current_user = request.user
        user_rated_movies = [
            rating.movie for rating in current_user.user_rating_set.all()]
    ...
    def get_queryset(self):
        ...
        for filtr in self.request.GET:
            if filtr == "movie":
                q &= (Q(name_eg__icontains=self.request.GET.get(filtr)) |
                      Q(name_ar__icontains=self.request.GET.get(filtr)))
                ...
                if self.request.GET.get(filtr):
                    ...
                    for actor in actors:
                        ...
                        q &= (Q(actors_name_eg__icontains=actor) |
                              Q(actors_name_ar__icontains=actor))
...
def movie_details(request, pk):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        if json.loads(request.body)['rating'] != '0':
            rating = User_Rating(user=current_user, movie=movie, rating=float(
                json.loads(request.body)['rating']))
            rating.save()
